...
#from audio_feedback import play_sound, speak_text

DATABASE_PATH = 'wow_cards.db'
...
def play_audio(sound_type, card_name=None):
    logging.info(f"Attempting to play sound for type: {sound_type}")
    if sound_type == 'new':
        PlaySound('new_match.mp3')
        if card_name:
            speak(card_name)
    elif sound_type == 'old':
        PlaySound('old_match.mp3')
        if card_name:
            speak(card_name)
...
def check_existing_card(card_id, variant_id, instance, card_name):
    """Check if the card already exists in the collection_inventory table"""
    with sqlite3.connect(DATABASE_PATH) as conn:
        cur = conn.cursor()

        cur.execute("SELECT * FROM collection_inventory WHERE card_id = ? AND variant_id = ? AND instance = ?", (card_id, variant_id, instance))
        result = cur.fetchall()

        if result:
            logger.info(f"Card with card_id={card_id}, variant_id={variant_id}, instance={instance} already exists in the database.")
            # Play 'old match' sound and announce the card name
            play_audio("old_match.mp3")
            speak(card_name)
        else:
            logger.info(f"Card with card_id={card_id}, variant_id={variant_id}, instance={instance} doesn't exist in the database.")
            # Play 'new match' sound and announce the card name
...
def query_database(texts):
    ...
    # If card name is not unique, query the database for block names for each set
                cursor.execute("SELECT DISTINCT set_abbreviation, block_name FROM card_versions WHERE card_name=?", (potential_card_name,))
        result = cursor.fetchall()

# Prepare a dictionary to map set abbreviations to block names
    set_to_block_mapping = {row[0]: row[1] for row in result}

    # Prompt the user to choose the correct set by displaying both the set name and its associated block name
    chosen_set_abbreviation = choose_set_abbreviation(list(set_to_block_mapping.keys()))
    for idx, (set_abbreviation, block_name) in enumerate(result, 1):
        print(f"{idx}. {set_abbreviation} - {block_name}")

    # Get user input
    choice = int(input("Enter the number corresponding to your choice: "))
    chosen_set_abbreviation = result[choice-1][0]
    ...
    if not check_existing_card(card_id, variant_id, instance, conn):
        logging.info("Determined as NEW card. Playing 'new_match' sound and saying card's name.")
        play_audio('new_match.mp3')
        speak(f"{card_name} new")
    else:
        logging.info("Determined as OLD card. Playing 'old_match' sound and saying card's name.")
        play_audio('old_match.mp3')
        speak(f"{card_name} old")
...
def choose_set_gui(set_to_block_mapping):
    ...
    top = Toplevel(root)
    top.title("Choose a set")
    
    chosen_set = tk.StringVar(value=None)

    # Create a label to instruct the user
    label = tk.Label(top, text="Multiple sets found for the card name. Please choose the correct set:")
    label.pack(padx=10, pady=10)

    # Create a button for each set
    for set_abbreviation, block_name in set_to_block_mapping.items():
        btn_text = f"{set_abbreviation} - {block_name}"
        btn = tk.Button(top, text=btn_text, command=lambda s=set_abbreviation: chosen_set.set(s))
