...
def check_existing_card(card_id, variant_id, instance, card_name):
    ...
    logging.info(f"Checking if card_id: {card_id}, variant_id: {variant_id}, instance: {instance} exists in collection_inventory")
    cur = conn.cursor()
    cur.execute(f"SELECT * FROM collection_inventory WHERE card_id = {card_id} AND variant_id = {variant_id} AND instance = {instance}")
    if cur.fetchone()[0] == 0:
        logging.info("Card is new.")
        play_audio("new_match.mp3")
        speak("New card added: " + card_name)
        return False
    else:
        logging.info("Card is old.")
        play_audio("old_match.mp3")
        speak("Duplicate card: " + card_name)
        return True
