...
logging.info("Connecting to the 'wow_cards' SQLite database for preselection.")
conn = sqlite3.connect('wow_cards.db')
cursor = conn.cursor()
# Fetch all unique set abbreviations
cursor.execute("SELECT DISTINCT set_abbreviation FROM card_versions")
all_set_abbreviations = [row[0] for row in cursor.fetchall()]

# Fetch all unique variant IDs
cursor.execute("SELECT DISTINCT variant_id FROM possible_card_variants")
all_variant_ids = [row[0] for row in cursor.fetchall()]
...
def get_user_preselections(all_set_abbreviations, all_variant_ids):
    
    def on_start():
        # Retrieve the selected values
        chosen_set.set(combo_sets.get())
        chosen_variant.set(combo_variant.get())
        root.quit()

    root = tk.Tk()
    root.title("Preselect Card Details")

    # For Set Selection
    label_sets = tk.Label(root, text="Choose a Set:")
    label_sets.pack(pady=10)
    combo_sets = ttk.Combobox(root, values=["Determine Dynamically"] + all_set_abbreviations)
    combo_sets.current(0)  # set the default value
    combo_sets.pack(pady=10)

    # For Variant ID Selection
    label_variant = tk.Label(root, text="Choose a Variant ID:")
    label_variant.pack(pady=10)
    combo_variant = ttk.Combobox(root, values=["Determine Dynamically"] + all_variant_ids)
    combo_variant.current(0)
    combo_variant.pack(pady=10)
    ...
    chosen_set = tk.StringVar()
    chosen_variant = tk.StringVar()

    root.mainloop()

    return chosen_set.get(), chosen_variant.get()

# Call the function at the start of the program
preferred_set, preferred_variant = get_user_preselections(all_set_abbreviations, all_variant_ids)
...
def query_database(texts):
    ...
    if card_name and not set_id:
        if preferred_set != "Determine Dynamically":
            matched_abbreviation = preferred_set
        else:
...
# If there's only one variant and it's variant_id 5, use it directly
    if len(variants) == 1 and variants[0][0] == 5:
        ...
        if preferred_variant != "Determine Dynamically":
            variant_id = preferred_variant
        else:
            if preferred_variant != "Determine Dynamically":
                variant_id = preferred_variant
            else:
